var YL = {
    	...
    	init:function(){
    		...
    		/*if (bId == null || bName == null || bId.length == 0 || bName.length == 0) {
    			$.get(YL.ctx+"/branch", function(result) {
    				if (result.length == 1) {
    		...
    		/*$(".each-zone a").click(function() {
    			var branchName = $(this).text().replace("找工作","") + "站";
    			$("#c_branch_name").text(branchName);
    			...
    			if (branch != null && branch != "") {
    				...
    				$.post(YL.ctx+"/branch", {
    					...
    					if (result == "success") {
    						window.location.href = window.location.href.replace("search_EQ_city.id","").replace("search_EQ_countyid","");
    					}
    				});
    			}
    		});*/
	...
	/*			$("#keywordSignUp").autocomplete(YL.areas,{
            		minChars: 0,
            		max: 5,
            		autoFill: true,
            		matchContains: true,
				...
				$('.code').keyup(function(){
					...
					if (code.length != 4) {
						...
						$(".btn-g-out").addClass("disabled");
						$(".btn-login-reg").attr("disabled","disabled");
					}else{
						$(".btn-g-out").removeClass("disabled");
				...
				$('.btn-login-reg').click(function(){
					...
					var code = $('.code').val();
					$.post(YL.ctx+'/register/fastLoginOrReg',{mobile:mobile,thekey:code},function(result){
						if(result == "error"){
							$(".code").focus();
							$('.code-error').html("验证码错误！");
						}else if(result == "success"){
							var type = $("input[name='type']").val();
							location.href=YL.ctx+"/zhaopin_"+_jobBaseId+".html?type="+type;
						}
					});
				});
				/*底部tab切换*/
    			...
    			$(window).scroll(function(){
    				if($(this).scrollTop()>$('#scroll-menu').offset().top){
    					$('.top-fixed').addClass('on');
    					$('.tab-g-title .btn').show();
    				}
    				else{
    					$('.top-fixed').removeClass('on');
    					$('.tab-g-title .btn').hide();
    				}
    			});
    			/*网友问答*/
    			$.get(YL.ctx+"/job/advisories/"+_jobBaseId,function(data){
    	    		...
    	    		$.each(data.content,function(i,j){
    	    			if(i<10){
    	    			divs.push('<div class="q-each"><ul class="ask-each">');
    	        		divs.push('<li class="name"><span class="mr10">'+(j.user.username == null ?"优蓝网友":j.user.username)+'</span>在&nbsp;'+j.createTimeString+' 问道：</li>');
    	        		divs.push('<li class="question">'+j.introduction+'</li>');
    	        		divs.push('</ul>'); 
    	        		if(j.replyContent != null && j.replyContent != ""){
    	        			divs.push('<ul class="answer-each">');
    	        			divs.push('<li class="pic50"><img src="'+YL.ctx+'/static/images/datas/about/wangli.png" alt="优蓝客服" /></li>');
    	        			divs.push('<li class="name">优蓝网客服 <label class="ml10">在&nbsp;'+j.replyTimeString +' 回复：</label></li>');/*'+j.replyUser.userName+'*/
    	        			divs.push('<li class="reply">'+j.replyContent+'</li>');
    	        			divs.push('</ul>');
    	        		}
    	        		divs.push('</div>');
    	    			}
    	    		...
    	    		$("#advisory").append(divs.join(''));
    	    		$(".advisory-count").text(data.totalElements);
    	    	});
    			/*提交网友问答*/
    			...
    			$('.btn-large').click(function() {
    				...
    				var nativeProvinceid=$("input[name='nativeProvinceid']").val(); 
    				var nativeCityid=$("input[name='nativeCityid']").val(); 
    				var nation=$("input[name='nation']").val(); 
    				var intention = $("input[name='intention']").val();
    				var jobTarget=$("input[name='jobTarget']").val();
    				$(".tip").empty();
    				if (name!=undefined&&!e_reg.test(name)) {
    					$('.name_err_tip').html('请输入您的姓名');
    					$("#input_name").focus();
    				} else if (name!=undefined&&!name_reg.test(name)) {
    					$('.name_err_tip').html('姓名为2-5位的汉字或3-12位的英文');
    					$("#input_name").focus();
    				}  else if (gender!=undefined&&!e_reg.test(gender)) {
    					$('.gender_err_tip').html('请选择您的性别');
    				} else if(idCard!=undefined&&!idcard_reg.test(idCard)){
    					$('.idCard_err_tip').html('请输入正确的法人身份证号码！');
    					$("#input_idCard").focus();
    				} else if (education!=undefined&&!e_reg.test(education)) {
    					$('.education_err_tip').html('请选择学历');
    				} else if (nativeProvinceid!=undefined&&!e_reg.test(nativeProvinceid)) {
    					$('.native_err_tip').html('请选择籍贯(省)');
    				} else if (nativeCityid!=undefined&&!e_reg.test(nativeCityid)) {
    					$('.native_err_tip').html('请选择籍贯(市)');
    				} else if (nation!=undefined&&!e_reg.test(nation)) {
    					$('.nation_err_tip').html('请选择民族');
    				} else if (jobTarget!=undefined&&(jobTarget==null||jobTarget=="")) {
    					$('.jobTargetProvinceCity_err_tip').html('请选择意向职位');
    				} else if (intention!=undefined&&(intention==null||intention=="")) {
    					$('.intention_err_tip').html('请选择意向城市');
    				} else {
			...
			userGainkey :function(type,loginname){
				$('.txt_user_loginname').val(loginname);
		   		...
		   		if(m_reg.test(mobile)){
		   			...
		   			$.post(YL.ctx+"/register/sendCode",{mobile:mobile,type:type},function(data){
		   				if(data=="success"){
		   					...
		   					var timmer = setInterval(function(){
		   						...
		   						if(thenum==-1){
		   							...
		   							$(".send-voice").attr("onclick","YL.job.detail.sendVoice()");
		   				   			$(".send-btn").attr("onclick","YL.job.detail.sendCode()");
			...
			signUp		:function(type,jobid){
				$.post(YL.ctx+"/signup/job",{jobid:jobid,type:type},function(result){/*报名和预约*/
					if(result=="success"){/*报名成功*/
						if(type==2){
							$('.success_span').html('报名成功');
							$('.success_p').html('恭喜您！报名成功');
						}
						YL.job.detail.resetDiv();
						$('.success').show();
						YL.job.detail.resize_layer();
		...
		list:{
			...
			init:function(){
			    ...
			    var dName = $("#dName").val();
			    var cName = $("#cName").val();
			    $("#sdName").text($("#sdName").text().replace("站",""));
			...
			clickJobType:function(id,typeName){
				...
				$.get(YL.ctx+"/job/findJobTypesByParent",{"parentId":id},function(result){
					if(result.length > 0){
						var subs = [];
						subs.push("<a href='javascript:void(0)'>所有</a>");
						$.each(result,function(i,j){
							if(j.jobName==$("#hsubJobType").attr("data")){
								subs.push("<a class='current sub-current' href='javascript:void(0)'>"+j.jobName+"</a>");
							}else{
								subs.push("<a href='javascript:void(0)'>"+j.jobName+"</a>");
							}
						});
						$(".sub_job_types").empty();
						/*二级标签显示地点*/
						...
						var light_flag = Math.ceil(typeindex / 8);
						if(typeindex%8==0){
							light_flag = Math.ceil((typeindex+1) / 8);
						}
			...
			searchJob:function(){
				...
				if(lables!=null && lables!=""){
			        ...
			        condition.push(lables);
			    }else{
			    	condition.push("0");
			    }
				if(jobType!=null && jobType !=""){
			   		condition.push(jobType);
				}else{
					condition.push("0");
				}
				...
				window.location.href=YL.ctx+"/zp_"+sortType+"_" + condition.join("_")+"_1.html";
			},
			...
			init:function(_companyId){
				$('.tab-title li').click(function(){
					...
					$('.fold-and-open').click(function(){
						$('.company-basic-part2 ul').toggleClass('height-auto');
